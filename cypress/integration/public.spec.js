describe('Hackathon 2 Public Test', () => {
  ...
  it('3-(1) (10%)', () => {
    cy.request('localhost:4000/api/getPostDetail?pid=1b9d6bcd-bbfd-4b2d-9b5d-ab8dfbbd4bed')
    .then((response) => {
      expect(response.status).to.eq(200)
      expect(response.body).to.have.property('message')
  ...
  it('5-(1) (10%)', () => {
    cy.request({
      method: 'DELETE',
      url: 'localhost:4000/api/deletePost?pid=228d96c4-fc74-4a7a-86ea-aeeb7f574d69',
      failOnStatusCode: false
    })
    .then((response) => {
      expect(response.status).to.eq(200)
